import { ProtectedLayout } from "@/components/layout/ProtectedLayout";
  ...
  export function CodingChallenges() {
    ...
    const handleStartChallenge = (challengeId: number, title: string) => {
      toast({
        title: "Challenge Started",
        description: `You've started: ${title}`,
      });
      // In future, navigate to challenge detail page
      // setLocation(`/student/challenge/${challengeId}`);
    };
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {CHALLENGES.map((challenge) => (
              <Card key={challenge.id} className="flex flex-col shadow-sm hover:shadow-lg transition-shadow border-border/50 group" data-testid={`card-challenge-${challenge.id}`}>
                <CardHeader>
