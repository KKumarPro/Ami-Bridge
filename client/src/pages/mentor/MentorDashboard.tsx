...
import { Loader2, MessageSquare, Star, FileText, LayoutDashboard, Users } from "lucide-react";
  ...
  return (
    <ProtectedLayout allowedRoles={["mentor"]}>
      <div className="space-y-8">
        <div>
          <h1 className="text-3xl font-bold font-display tracking-tight">Mentor Portal</h1>
          <p className="text-muted-foreground mt-1">Manage your assigned students and track their progress.</p>
        </div>
        ...
        <Tabs defaultValue="dashboard" className="space-y-6">
          ...
          <TabsContent value="dashboard" className="space-y-8">
            {topScorer ? (
              <Card className="bg-gradient-to-br from-primary to-primary-foreground text-white shadow-xl border-none overflow-hidden">
                <CardContent className="p-8 flex items-center justify-between relative">
                  <div className="relative z-10">
                    <p className="text-primary-foreground/80 font-bold uppercase tracking-widest text-xs mb-2 text-white/80">Top Performer</p>
                    <h2 className="text-4xl font-black font-display mb-1 text-white">{topScorer.name}</h2>
                    <p className="text-primary-foreground/60 text-white/60">{topScorer.profile?.branch || "General"}</p>
                  </div>
                  <div className="text-right relative z-10">
                    <p className="text-5xl font-black text-white">{topScorer.profile?.prScore || 0}</p>
                    <p className="text-xs font-bold uppercase tracking-widest text-primary-foreground/80 text-white/80">Total PSR Score</p>
                  </div>
              ...
              <Card className="bg-muted/30 border-dashed">
                <CardContent className="p-12 text-center text-muted-foreground">
                  <p>No students assigned to calculate top performer.</p>
                </CardContent>
              </Card>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <Card className="shadow-sm">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Average PSR</CardTitle>
                </CardHeader>
                <CardContent>
          ...
          <TabsContent value="students">
            <Card className="shadow-sm">
              <CardHeader>
                ...
                <CardDescription>Review performance and provide feedback.</CardDescription>
              </CardHeader>
              <CardContent>
